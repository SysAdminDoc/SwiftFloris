...
**Last consolidated:** 2026-06-05 (from the autonomous research run at
...
| `FLAG_SECURE` on password/incognito fields | `FlorisImeService.applyFlagSecureForCurrentField` plus the incognito toggle callback (N7.2, R7-1) |
...
**v1.8.104 - v1.8.247** ships the seventh research-pass privacy,
...
v1.8.171 through v1.8.247 refresh the release front door after the 2026-05-31
...
v1.8.230 freezes the sync sealed-box v1 envelope before transport activation:
fixed X25519 key vectors pin the ephemeral-public-key, derived nonce, and
AES-GCM ciphertext/tag bytes, while the threat model documents the compatibility
policy for future envelope/KDF changes.
v1.8.231 re-applies the IME window `FLAG_SECURE` policy as soon as dynamic
incognito is toggled, so ordinary fields become screenshot-blocked immediately
when a user starts a private session from the smartbar.
v1.8.232 gives Settings -> Personal dictionary operation-blocked system-back
gestures explicit visible feedback for save/delete/import/export work while
preserving the existing leave-blocking policy and progress cards.
v1.8.233 narrows editor `InputConnection` batch-edit critical sections:
selection, commit, composing-finalize, and composing-region replacement paths
now compute/push expected content before opening the batch, and a tested helper
pairs begin/end with `try/finally`.
v1.8.234 backfills the fragile post-v1.8.225 hotfix test surface: Arabic
combining-mark shaping, Snygg unknown selectors, `contentScale` values,
private-session trace suppression, and locale-scoped personal n-gram flush
behavior now have focused guards.
v1.8.235 closes the Settings search TalkBack pass: the field carries label and
state semantics, result-status changes use polite live regions, result rows
announce position/screen/title/summary context as buttons, and the accessibility
manual-QA checklist covers the flow.
v1.8.236 closes the suggestion privacy request-snapshot gap: `NlpManager.suggest`
now freezes incognito, no-personalized-learning/editor sensitivity, enabled
flags, offensive-content preference, and emoji count inputs before async
provider, trace, and smart-compose ghost-text work begins.
...
removed.
v1.8.242 closes the native string ByteBuffer slice gap: `NativeStr.toJavaString`
now decodes only the remaining bytes through a duplicated view and preserves the
caller-visible buffer position for heap, sliced heap, direct, and read-only
buffers.
v1.8.243 closes the localization content-quality pass: Turkish adjacent
...
v1.8.247 closes R16-1 by making manual subtype switching by id resolve one
nullable subtype and no-op for stale ids instead of checking one subtype-list
snapshot and force-unwrapping a second lookup. The same release keeps
pre-Android 12 Auto Backup lint-clean by relying on the existing allowlist for
learned typing files while Android 12+ explicit data-extraction excludes remain
pinned.

...
- Previous-word and trailing-language evidence is now sentence-local, so a prior Spanish/French/etc. sentence does not keep biasing the next sentence after `.`, `!`, `?`, or newline.
...
5. Multilingual detection should keep adding context.
   Current support now checks the current token across active locales, lowers wrong-language correction confidence when a token is valid elsewhere, and uses sentence-local trailing-word locale evidence to steer partial completions toward the active sentence language. The next step is shared-spelling handling, same-sentence language switches, and broader bilingual replay fixtures from real traces.
...
- Add deterministic replay tests for shared-spelling bilingual words, same-sentence language switches, and more multi-word repairs; partial phrase completions, bilingual context completions, sentence-boundary context reset, a first multi-word repair, and longer Flow contraction rescue are now covered.

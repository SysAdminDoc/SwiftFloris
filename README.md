...
| **Clipboard** | Room-backed history with pinning + per-app source tag, media/provider metadata, sensitive-item gates, long-press marking and four-second per-entry reveal with lock-screen and incognito blocks, a 64 KiB UTF-8 text-retention boundary with direct-paste-only handling for larger live clips, startup/restore reconciliation, in-keyboard text search with type-filter composition, TalkBack labels for image/video media history tiles, and passphrase-encrypted portable backups whenever clipboard data is included | On-device; encrypted before export |
| **Password input** | Optional digit scrambling for numeric password fields, generated once per field focus so the visible key, TalkBack label, and committed digit stay aligned | On-device |
| **Input feedback** | Optional local sounds for standard, delete, return, and spacebar keys, with pooled playback and system effects retained as the default | On-device; files stay local |
| **Gestures** | Spacebar touchpad mode moves the cursor on both axes with a saved movement ratio, while the existing discrete swipe actions remain available | On-device |
...
| **MCP daemon bridge** | AIDL and trust-policy contracts plus a Settings review surface for user-installed MCP daemons; production binding is paused until a live audited keyboard dispatch action exists | No daemon is bound from keyboard startup; when enabled, a package may hold only allowlisted permissions: network, SMS, Bluetooth and nearby-devices transports are all rejected before trust or binding |
| **Addon packs** | Addon manifest/enumerator contracts, IME-startup registry reconciliation, Settings -> Addons status/rescan, explicit trust for non-co-signed addons, trust reset/changed-certificate controls, dictionary-pack catalog details, persisted signing-certificate pins, descriptor validation, provenance reports, runtime bundle-size rejection reasons, typed dictionary-pack catalog, and addon APK dictionary asset mounting | Permission allowlist at enrolment |
| **Settings UX** | Premium dashboard-style five-bucket Settings home (Typing experience, Personalization, Privacy & data, Advanced, About) with compact no-cloud/no-telemetry/offline trust chips, a primary setup action, search rail, local privacy/build/sync signals, grouped navigation cards, pinned migration/privacy/about actions, global Settings search with accent-insensitive matching, first-open focus, clear action, Search IME action, no-results path back to all settings, synonym hits for dark theme, haptic, trace, punctuation, and privacy queries, result-list scroll reset, TalkBack labels/live result-status/result-row context, and destination search results that scroll to and briefly highlight their matching preference row; per-app keyboard profile editor for package-specific accent, incognito, clipboard, suggestion, and Smartbar gesture overrides with undo for profile deletion; clearer empty states for voice setup, selected user-dictionary languages, extension categories, language packs, filtered clipboard history, and theme-manager recovery; user-dictionary back feedback during active save/delete/import/export work; surfaced keyboard preview field with ready/active state feedback | Local UI only |
...
| **Sync scaffold** | Transport-neutral personal-dictionary sync model with QR pairing payloads, authenticated X25519/AES-GCM sealed-box envelopes, CRDT merge tests, persisted sync identity, and Settings export/import actions for manual JSON files or picked local SAF folders | No network; user-chosen local file channel |
...
| **Local release evidence** | `scripts/release-evidence.ps1` discovers and runs every `scripts/test-*.py` self-test, wires every `scripts/check-*` / `scripts/verify-*` gate, and records explicit operator-run reasons for the connected-device baseline and benchmark gates alongside release-front-door, Fastlane metadata, backup/privacy copy, fork identity, layout JSON, locale coverage, public-doc/F-Droid version pins, the semantic trust-capability gate, release developer-tool isolation, repo hygiene, root-crash-log, Kotlin build-cache CVE guard, production `runBlocking` allowlist, target-SDK 37 shadow, no-network, data-extraction, cache-disabled Gradle unit-test / lint / release-assemble gates, sample addon APK validation, OSV severity, security-dependency freshness, and reproducible-APK gates in `build/release-evidence/<timestamp>/`; high-risk SDK, permission, storage, AIDL, dependency, runtime, and accessibility assertions are normalized in `app/src/main/config/trust-capabilities.json`; startup crash recovery routes through the local crash dialog; restore/crash diagnostics use project logging with safe fallback copy; Roborazzi visual-regression checks use committed setup, search, settings, theme, and Addons baselines; Macrobenchmark trace sections cover 6 hot paths; dependency freshness is pinned through Compose BOM 2026.08.00 / KSP 2.3.11 / AboutLibraries 15.1.0 / Roborazzi 1.72.0 / material-kolor 5.0.0 | Audit-friendly |
...
Clipboard-bearing backups use the versioned `.sfbak` format and require their
passphrase during restore; SwiftFloris cannot recover a forgotten passphrase.
Backups that exclude clipboard history remain standard `.zip` files. Earlier
SwiftFloris and FlorisBoard `.zip` backups remain restorable and are clearly
identified as unencrypted during review. Encrypted archives are authenticated
before import, and each restore attempt keeps an app-private recovery snapshot
so cancellation or a failed selected section rolls live data back.
...
# JDK 21+
# Gradle 9.7.1 (use the bundled wrapper)
...
SwiftFloris ships the MCP (Model Context Protocol) AIDL client, trust-policy contracts, and Settings review surface. Production daemon binding and tool dispatch are paused until a live audited keyboard action exists; no MCP invocation can occur from keyboard startup. **Settings → MCP daemon bridge says so on the screen**, because the trust review, the bridge switch and the per-daemon toggles are all still there and would otherwise read as controls over something running. Your choices are saved and apply once binding returns. One flag, `McpBindingPolicy`, decides this for both the keyboard service and the Settings screen. MCP daemon packages are a separate trust boundary, so discovery rejects packages requesting `INTERNET`, network-state, or Wi-Fi-state/change permissions before certificate trust, registration, or binding; Settings shows the rejected package and permission. When dispatch is enabled by a future live action, tool invocations will route through the same `SensitiveFieldGuard` and `NlpAddonHub` as smart-compose and translation.

The full bridge contract spans `IMcpDaemon.aidl` (Binder surface), `AndroidMcpClient` (JSON envelope translation), `McpServiceConnectionManager` (per-daemon bind lifecycle), `McpAndroidDiscoverer` (PackageManager discovery), `McpDispatchRouter` (registry → consent/disable/sensitive gates → tool → response), and the Settings screen that reviews daemon trust and per-daemon switches. The live binding entry point remains intentionally disabled until that route has a real keyboard caller.
...
- **Security behavior tests:** encrypted dictionary storage, private dictionary learning, clipboard policy, backup, sync, MCP consent, and privacy gates are checked through observable behavior. The attached Android test lane covers the real Room stores.
- **Visual regression:** Roborazzi 1.72.0, plugin alias active. Run `:app:verifyRoborazziDebug` for committed baselines and `:app:verifyRoborazziRelease` before APK publication. Baselines cover the maintainer chip, SwiftKey High Contrast, Aurora Animated, and Settings -> Addons surfaces.
- **Plain unit-test report:** Roborazzi capture classes use explicit `Roborazzi...CaptureTest` names, so their capture-only status is visible alongside the ordinary JVM tests. Preview density and font scale are fixed for repeatable captures. Image comparison remains in the verify task.
...
- **targetSdk 37 shadow preflight:** `python scripts/verify-targetsdk37-shadow.py` runs the debug manifest, unit-test, and assemble lanes with `-PprojectTargetSdk=37`, verifies the merged manifest target, confirms the API 37 IME-window replay tests ran, and leaves the release `projectTargetSdk=36` pin unchanged.

...
- Bound the error text that reaches toasts and notice cards. Every path that shows a caught throwable to a person now routes through one helper that collapses line breaks and control characters to spaces and cuts the result at 200 characters. `ZipUtils` quotes a rejected archive entry name straight into its `SecurityException`, so a crafted archive previously chose up to 255 characters of what the user saw, newlines and bidi overrides included. The full cause still goes to the log, and the extension import and export paths now write one where they wrote nothing.

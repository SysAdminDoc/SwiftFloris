...
| Thumb / split | **Split renderer shipped.** `prefs.keyboard.splitKeyboardEnabled` + `ImeWindowMode.Fixed.SPLIT` + `Fixed.Split` constraint class + `SplitKeyboardLayoutCalculator` + `TextKeyboardSplitLayout` + `SplitGutterPostPass` now render a viable tablet split with gutter-aware touch routing. |
...
| P3 | **Split keyboard renderer** — per-row gutter, per-side key rects, gutter-aware touch routing | Preference + window mode + constraint class shipped v1.8.3; renderer + touch-gutter behavior shipped v1.8.62. | ✅ | Shipped | `TextKeyboardSplitLayout` + `SplitGutterPostPass`. |
...
| P9 | **Calendar quick-insert** — pick agenda entry, drop event details | `QuickAction.InsertCalendarEvent` shipped v1.8.64 with an IME-local agenda picker over local `CalendarContract.Instances`. | ✅ | Shipped | `CalendarQuickInsertManager` + `CalendarAgendaReader` + `CalendarAgendaPickerPanel`; `READ_CALENDAR` requested only after explicit tap. |
...
| P12 | **OneDrive learned-words backup** | Out by §1. Local-only replacement = personal-dictionary export to plain CSV/JSON + passphrase-encrypted SwiftFloris `.sfexp` + Syncthing sync. | 🟢 | Shipped | `EncryptedDictionaryExport` codec plus Settings **Export encrypted** / encrypted import round-trip shipped in v1.8.65. |
...
- ✅ **A3 (P12)** shipped 2026-05-17 (codec primitive v1.8.54; Settings round-trip v1.8.65). New pure-Kotlin `EncryptedDictionaryExport` envelope codec uses AES-256-GCM keyed by PBKDF2-HMAC-SHA-256 at the OWASP-2025 600 000-iteration count, with the iteration count baked into the 44-byte header so future bumps decrypt old exports unchanged. 16-byte random PBKDF2 salt + 12-byte random AES-GCM nonce per export. Cryptographic-indistinguishability collapse: wrong passphrase and tampered ciphertext both surface as `BAD_PASSPHRASE` so the UI can't accidentally leak which case it was. Header validation rejects oversized / truncated / unsupported-version / corrupt-header blobs before touching the cipher. Settings → Personal dictionary now exposes **Export encrypted**, prompts for passphrase + confirmation, launches a `.sfexp` create-document flow, and writes the encrypted envelope directly to the selected URI after scrubbing the plaintext bytes. Import now sniffs `SFEXP1`, asks for the passphrase only for encrypted files, decrypts in memory, and routes decrypted SwiftFloris combined-list payloads through `DictionaryImporter` + `PersonalDictionaryImportBatch` so the summary/rollback UX is preserved. `UserDictionaryCombinedListCodec` centralizes the legacy semicolon format. Tests cover codec round-trip/failures plus combined-list parse/detect/import routing and encrypted combined-list decrypt → importer round-trip.

### Phase B — Touch & decoder calibration (v1.8.52 – v1.8.61)
...
- ✅ **B1 (P13)** shipped 2026-05-17 (v1.8.60). `ColdStartNextWordPriors` now extends past the original English-only seed set with localized sentence starts plus one-word, two-word, and three-word phrase continuations for EN/CS/DE/ES/FR/IT/PT. Added top-1,000 `rspeer/wordfreq` 3.1.1 Zipf seed overlays at `assets/freq/cs.tsv`, `de.tsv`, `es.tsv`, `fr.tsv`, `it.tsv`, and `pt.tsv`, mirroring the existing `freq/en.tsv` parser shape. `ZipfFrequencyTableTest` now parses every bundled multilingual seed file and pins the 1,000-row contract. Full corpus-sized SUBTLEX tables stay in dictionary-pack addons so the base APK remains lean.
- ✅ **B2 (P18)** shipped 2026-05-17 (v1.8.61). Quick-prediction-insert now has an explicit `QuickPredictionInsertTuning` floor (`minWeightedConfidence = 0.40`, `maxContextRecencyBoost = 0.35`). For blank-current-word insertion, the center/default word candidate fires only at cold start, after `.`, `!`, `?`, or after newline when `candidate.confidence * (1 + contextProbability * recencyBoost)` clears the floor. `NlpManager.shouldSuppressPlainSpaceForPrediction()` delegates to the same ranker decision, so a rejected low-confidence prediction no longer swallows the user's normal space. Added property-style coverage sweeping context × candidate confidence × recency plus focused floor / boundary tests.
- ✅ **B3 (P19)** shipped 2026-05-17 (v1.8.55). New asymmetric branch in `MultilingualTokenScorer`: when the typed word is shared across ≥ 2 active locales AND the candidate is recognised in only one, push the score to `SharedSpellingOneLocaleCandidateConfidence = 0.30` — intentionally sub-floor relative to `SwiftKeyCandidateRanker.MinAutoCommitLanguageConfidence = 0.40` so spacebar autocommit refuses the one-language overwrite. When typed is shared AND candidate is also shared, keep the existing 0.52 dampening (the candidate could plausibly fit either side of the bilingual sentence). Existing literal-protection (`candidateMatchesTypedWord` → 1.0) unchanged. 2 new unit tests pin the both-shared and literal-protection siblings; the existing "shared-spelling dampening" test was reframed and now asserts `< 0.40` to pin the relationship to the ranker's autocommit floor. Existing trace fixture stays valid (0.30 is even more below the floor than 0.52 was); fixture regen is a B5 follow-up.
...
### Phase D — Productivity surfaces (v1.8.57 – v1.8.64)
...
- ✅ **D1 (P9)** shipped 2026-05-17 (v1.8.64). New `QuickAction.InsertCalendarEvent` smartbar action reads from `CalendarContract.Instances` (today + next 7 days), shows an IME-local `CalendarAgendaPickerPanel`, and inserts the selected event title + localized date/time at the cursor. Permission: `READ_CALENDAR`, requested only after explicit tap through `CalendarPermissionActivity`. Registered in `QuickActionJsonConfig`; existing and new smartbar arrangements expose Calendar from the hidden editor pool.
- ✅ **D2 (P10)** shipped 2026-05-17 (v1.8.58). New `QuickAction.InsertTask` (`data object`, `@SerialName("insert_task")`) uses the cross-app `Intent.ACTION_SEND` + `Intent.createChooser` pattern (instead of the narrower `ACTION_INSERT` the parity roadmap originally suggested) so any installed task / note app that registers a `SEND` filter surfaces — Tasks.org, OpenTasks, Google Tasks, Joplin, Notion, Markor, etc. — without SwiftFloris needing to hard-code per-app contracts. `EXTRA_TEXT` carries the editor's current selection (or the last 140 chars of text-before-selection when nothing is selected) so the destination app's "new task" form opens pre-filled. `SensitiveFieldGuard` gates the action: password / numeric-PIN / no-personalised-learning fields refuse to forward via the share sheet even on explicit tap, with a Toast explaining why. `FLAG_ACTIVITY_NEW_TASK` set because the IME service isn't an Activity; `ActivityNotFoundException` falls back to a "install a task app" Toast. Registered in `QuickActionJsonConfig` polymorphic codec (and also added `TranslateSelection` to the same registration which was previously relying on sealed-class auto-discovery). Display name "Add task" and tooltip "Send current selection to a task / note app (Tasks.org, OpenTasks, Google Tasks, Joplin, etc.)" — both hard-coded English in this slice (matching the existing `TranslateSelection`); Crowdin resources follow in a localization sweep.
- ✅ **D3 (P26)** shipped 2026-05-17 (v1.8.59). `CorrectionOutcomePriors` now keeps a bounded weekly metadata ledger in the same TSV file as the existing accepted/rejected pair priors (`#week` rows, old five-column rows still load). `accuracyDelta()` compares current-week accepted corrections with the previous week and reports the count, percent change, and trend (`NO_BASELINE` / `FEWER` / `MORE` / `UNCHANGED`). Settings → Typing → Typing stats surfaces this as "Accepted corrections this week" with no-data, no-baseline, fewer, more, and unchanged summaries. No new permissions, dependencies, or network surfaces.
...
- **Phase D**: Calendar quick-insert works on at least two AOSP calendar apps + Google Calendar. Tasks quick-insert resolves on Tasks.org and Google Tasks. Accuracy-delta number computes deterministically from a synthetic `CorrectionOutcomePriors` fixture. **Code-side D1/D2/D3 shipped; manual calendar-app matrix remains a device QA follow-up because this VM lacks Android runtime access.**

...
- Verified: live docs still route contributors to missing canonical files. `CHANGELOG.md`, `PROJECT_CONTEXT.md`, and `.github/workflows` are absent, while `CONTRIBUTING.md`, several docs, and README live sections still reference them. Canonical tracked release notes are README release bullets plus `fastlane/metadata/android/en-US/changelogs/*.txt`.

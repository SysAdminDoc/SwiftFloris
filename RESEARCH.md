...
- Verified: live docs still route contributors to missing canonical files. `CHANGELOG.md` and `PROJECT_CONTEXT.md` are absent (gitignored local-only), while several docs and README live sections still referenced them. Canonical tracked release notes are README release bullets plus `fastlane/metadata/android/en-US/changelogs/*.txt`. Fixed in live-doc integrity checker pass.

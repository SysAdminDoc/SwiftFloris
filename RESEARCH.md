...
5. **Refuted on 2026-09-05, was wrong when written.** This pass claimed the resize gesture read stale row counts. It does not. `rowCount` and `smartbarRowCount` are local `by` delegates over the `State<Int>` that `FlorisImeSizing.rowCountAsState()` returns, so a lambda capturing them captures the *State object* and its reads inside `pointerInput` are live. The four gesture callbacks are recreated per recomposition but close only over the window controller and a constant handle enum, so a frozen instance behaves identically. An audit of all eighteen `pointerInput(Unit)` sites, checked against the decompiled `SuspendingPointerInputModifierNodeImpl` in compose-ui 1.12.0, found the real instances of this defect elsewhere: a frozen `base` emoji in `EmojiPaletteView.kt` and a frozen `longPressDelay` in `CandidatesRow.kt`. Both are fixed; the roadmap item this finding created was deleted.

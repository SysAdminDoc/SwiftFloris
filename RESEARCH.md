...
- **Test and doc gaps carried forward unchanged:** 76 source-text assertion sites across 20 test files; 32 Roborazzi tests assert nothing under `:app:test`; settings-search index covers ~27%; shared widgets at 44 dp; `crowdin.yml` still names upstream's `FSEC_*` env vars with no consumer (the ownership Open Question is untouched and still gates the i18n item); `docs/PRIVACY_AND_AI.md:236-237` still misstates the dictionary key location (the THREAT_MODEL system-dictionary claim, by contrast, was made true by `e18a59578`).

...
3. A new `## 0. Research Refresh v5.6` section appended to
...
| **#12** — `VoiceInputSetupActivity` exported by default (no manifest verify on `android:exported`); accepts arbitrary `Intent` extras | Low | Manifest hardening; could be its own slice (v1.8.111+ if maintainer wants to follow up). |
...
| Finding | Severity | Disposition |
|---|---|---|
| **#2** — no `SecurityException` catch when reading a foreign content URI; phantom history entries pointing at non-existent files | Medium | One-PR-shaped fix; flag for v1.8.111+. |
| **#3** — `ClipboardFileStorage.cloneUri` has no max-size cap (unlike the v1.8.85 ZipUtils + the extension-edit screen) | Medium | One-PR fix; pin a cap (e.g. 32 MB image, 128 MB video). |
| **#4** — no startup reconciliation between DB rows and on-disk files; `fallbackToDestructiveMigration` orphans every file forever | Medium | Modest slice; flag for v1.8.111+. |
...
| **#10** — backup-restore drops the `ClipboardFileInfo` row, so restored items have URIs pointing at IDs not in `clipboard_files` table → "Unable to resolve image" | Medium | One-PR fix on the restore path; flag for v1.8.111+. |
| **#12** — `openFile` doesn't pre-check `file.exists()` (FileNotFoundException to receiver) | Low | Defer. |
| **#13** — `revokeUriPermission` only on explicit delete, not on history rotation / expiry | Medium | Route eviction through `ClipboardManager.deleteClip`; bundle with #19's video extension. |
...
| **#16** — `uriToPreviewBitmap` modern (API 28+) branch has no max-size guard | Low | Bundle with image-decode hardening. |
...
## 5. Open follow-up roster (post-v1.8.110)

Items the seventh-pass audit surfaced but couldn't close in the
per-PR window. Priority-scored.
...
| G2 | `ClipboardFileStorage.cloneUri` max-size cap (e.g. 32 MB image, 128 MB video) | Clipboard #3 | 3 | 1 | 2 | **8.0** |
...
| G6 | `revokeUriPermission` on history rotation / expiry path | Clipboard #13 | 3 | 1 | 1 | **7.0** |
| G7 | `VoiceInputSetupActivity` `android:exported="false"` + Intent extras validation | Voice #12 | 2 | 1 | 1 | **5.0** |
...
| G12 | `uriToPreviewBitmap` modern branch max-size guard | Clipboard #16 | 2 | 1 | 1 | **5.0** |

High-leverage items (score ≥ 5.0): **G2, G6, G7, G8, G10, G12**.
These map naturally to v1.8.111 – v1.8.116 single-feature releases.
...
ROADMAP refresh (`v5.6`) and into
[`ROADMAP_RESEARCH_ADDENDUM_2026-05-17.md` §0.c](../../../ROADMAP_RESEARCH_ADDENDUM_2026-05-17.md).
The v5.6 ROADMAP append is the canonical user-facing record; this

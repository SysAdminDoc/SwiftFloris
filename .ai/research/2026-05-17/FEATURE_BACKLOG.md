...
| **HeliBoard dictionary downloader UI** for user-supplied n-gram packs | Partially shipped v1.8.81-v1.8.84 for the Next-10.3a/b/c/d addon catalog + persisted signing-pin + startup reconciliation + Settings status foundation (`AddonRegistry`, `DictionaryPackCatalog`, `AddonSigningPinSet`, `AddonRegistryStartup`, `AddonRegistryStore`, `AddonsSettingsScreen`, `prefs.addon.signingCertPins`); signing-pin revoke/reset UX and asset mounting remain |
...
| **Consolidate root-level `*MULTILINGUAL.md`, `VOICE_*.md`, `FUTO_VOICE_*.md` into `docs/`** | ✅ shipped docs-only 2026-05-17; multilingual gesture, multilingual research, FUTO voice troubleshooting, and voice-command docs moved under `docs/` with internal links updated |

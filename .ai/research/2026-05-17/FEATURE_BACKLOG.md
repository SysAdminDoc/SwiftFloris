...
| **HeliBoard dictionary downloader UI** for user-supplied n-gram packs | Partially shipped v1.8.81-v1.8.83 for the Next-10.3a/b/c addon catalog + persisted signing-pin + startup reconciliation foundation (`AddonRegistry`, `DictionaryPackCatalog`, `AddonSigningPinSet`, `AddonRegistryStartup`, `AddonRegistryStore`, `prefs.addon.signingCertPins`); Settings UI / install-hint list and asset mounting remain |

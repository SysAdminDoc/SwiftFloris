...
|------|-------------|------------|
| 2026-06-04 (v1.8.216) | Compatible freshness batch: Compose BOM `2026.05.01`, KSP `2.3.9`, Roborazzi `1.63.0`; deferred Kotlin `2.4.0` and AndroidX Core `1.19.0` | Applied the compatible pins after checking Maven Central / Google Maven metadata and running `:app:compileDebugKotlin` plus the focused settings-search unit test. Kotlin `2.4.0` stays deferred because no KSP `2.4.0` Gradle plugin artifact is published yet (latest KSP metadata is `2.3.9`). AndroidX Core `1.19.0` stays deferred because a smoke build failed `:app:checkDebugAarMetadata` with a `compileSdk 37` requirement while the repo intentionally remains on compileSdk 36 until the API 37 behavior-gate plan is ready. |
| 2026-05-18 (v1.8.139) | Dependency-warning review: Gradle `9.5.1`, Navigation Compose `2.9.8`, JUnit Vintage `6.0.3` | Applied after checking official Gradle 9.5.1 release notes plus `gradle.org/release-checksums`, AndroidX Navigation 2.9.8 release notes, and JUnit 6.0.3 release notes. Gradle stays checksum-pinned with `distributionSha256Sum=bafc141b...`; Navigation 2.9.8 is a stable bug-fix patch; JUnit Vintage 6.0.3 is test-runtime-only and still passes the JUnit-4-style Robolectric bridge despite Vintage's deprecation notice. Full local gate passed: `:app:testDebugUnitTest :app:lintDebug :app:assembleDebug`. |

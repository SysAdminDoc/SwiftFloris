...
**Last updated:** 2026-05-09 (v1.7.0)
...
## 3. Defenses (live in v1.7.0 unless flagged)
...
- Clipboard items are AES-256-GCM encrypted at rest (max 50 entries). The
  passphrase derives from `MasterKey` in AndroidKeystore, hardware-backed when
...
- APK signing fingerprint visible in Settings → About → APK signing fingerprint
  ...
  to detect a swap.
- Reproducible-build verification on F-Droid is the larger Now-item (N6.3) that
  closes this gap structurally.

### 3.6 CAKI (cross-app KeyEvent injection)
...
| Personal-dictionary Room DB is not currently SQLCipher-encrypted | Low (data is app-private; only matters for forensic recovery on a rooted device) | N7.4 |
